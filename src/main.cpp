...
HRESULT                     g_ModelLoadResult = S_FALSE;
...
HRESULT CreateModel();
void ShowModelLoadErrorMessage(HWND hWnd, HRESULT hr);
                      ...
                      _In_ int           nShowCmd)
{
    ...
    {
        std::thread(CreateModel).detach();
        //hr = CreateModel();
        if (FAILED(hr))
        {
            ShowModelLoadErrorMessage(g_MainWindow, hr);
        }
    }
